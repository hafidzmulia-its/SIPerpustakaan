...
<x-app-layout>
    ...
    <div class="p-4 sm:p-6 lg:p-8 font-sans text-accentDark">
        ...
        <div class="flex flex-col md:flex-row gap-8 mb-6">
            ...
            <div class="flex-1 flex flex-col gap-6">
                <!-- Greeting -->
                <div>
                    <h1 class="text-4xl font-extrabold text-accentDark">Manajemen Akun Petugas & Admin</h1>
                </div>
                    ...
                    <div class="flex items-center gap-3">
                        <div class="relative flex-grow">
                            ...
                            <input
                                ...
                                placeholder="Klik di sini untuk mencari akun..."
                                value="{{ $query }}"
        ...
        <div class="shadow-lg rounded-2xl overflow-hidden">
            <div class="bg-thead p-4">
                <p class="font-semibold text-white">Daftar Akun</p>
            </div>
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-left">
                        <thead class="bg-thead text-white">
                            <tr>
                                @php
                                    $columns = ['username' => 'Username', 'level_user' => 'Level', 'created_at' => 'Tanggal Bergabung'];
                                @endphp
                                @foreach($columns as $key => $title)
                                <th class="p-4 font-semibold">
                                    <a href="{{ route('users.index', array_merge(request()->all(), ['sort_by' => $key, 'sort_dir' => $sort_by == $key && $sort_dir == 'asc' ? 'desc' : 'asc'])) }}" class="flex items-center gap-2">
                                        {{ $title }}
                                        @if($sort_by == $key)
                        ...
                        <tbody>
                            @forelse($users as $user)
                                <tr class="border-b border-gray-300 {{ $loop->odd ? 'bg-sidebarBg' : 'bg-activeBg' }} text-accentDark">
                                    ...
                                    <td class="p-4">{{ ucfirst($user->level_user) }}</td>
                                    ...
                                    <td class="p-4 flex justify-end gap-3">
                                        <button class="p-2 bg-yellow-400 text-white rounded-md hover:bg-yellow-500 transition"
                                            x-data @click="$dispatch('open-edit', {{ $user->toJson() }})">
                                            <svg class="w-5 h-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" /></svg>
                                        </button>
                                        <form method="POST" action="{{ route('users.destroy', $user) }}" onsubmit="return confirm('Apakah Anda yakin ingin menghapus akun ini?')">
                                            @csrf
                                            @method('DELETE')
                            ...
                            @empty
                                <tr><td colspan="4" class="text-center p-6 text-gray-500">Tidak ada data akun.</td></tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
    ...
    {{-- Modal Create --}}
    <div x-data="{ open: false, level: 'petugas' }" x-show="open" @open-create.window="open = true" @close-modal.window="open = false" @keydown.escape.window="open = false" style="display: none" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
        <div @click.away="open = false" class="shadow-lg rounded-2xl overflow-hidden w-full max-w-md">
            <div class="bg-thead p-4"><h3 class="font-bold text-white">Tambah Akun</h3></div>
            <div class="bg-activeBg p-6">
                <form method="POST" action="{{ route('users.store') }}" class="space-y-4">
                    @csrf
                    <div>
                        <label class="font-semibold text-accentDark">Username</label>
                        <input type="text" name="username" class="mt-1 w-full p-2 bg-gray-200 border border-gray-300 rounded-lg text-gray-700 focus:ring-2 focus:ring-accentDark" required>
                    </div>
                     <div>
                        <label class="font-semibold text-accentDark">Password</label>
                    ...
                    <div>
                        <label class="font-semibold text-accentDark">Level</label>
                        <select name="level_user" x-model="level" class="mt-1 w-full p-2 bg-gray-200 border border-gray-300 rounded-lg text-gray-700 focus:ring-2 focus:ring-accentDark" required>
                            <option value="petugas">Petugas</option>
                            <option value="admin">Admin</option>
                            <option value="siswa">Siswa</option>
                        </select>
                    </div>
                    <template x-if="level === 'siswa'">
                        <div class="space-y-4">
                            <div>
                                <label class="font-semibold text-accentDark">Nama Siswa</label>
                                <input type="text" name="nama_siswa" class="mt-1 w-full p-2 bg-gray-200 border border-gray-300 rounded-lg text-gray-700 focus:ring-2 focus:ring-accentDark" :required="level === 'siswa'">
                            </div>
                            <div>
                                <label class="font-semibold text-accentDark">Kelas</label>
                                <input type="text" name="kelas" class="mt-1 w-full p-2 bg-gray-200 border border-gray-300 rounded-lg text-gray-700 focus:ring-2 focus:ring-accentDark" :required="level === 'siswa'">
                            </div>
                        </div>
                    </template>
    ...
    {{-- Modal Edit --}}
    <div x-data="{ open: false, user: {} }" x-show="open" @open-edit.window="open = true; user = $event.detail" @close-modal.window="open = false" @keydown.escape.window="open = false" style="display: none" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
        <div @click.away="open = false" class="shadow-lg rounded-2xl overflow-hidden w-full max-w-md">
            <div class="bg-thead p-4"><h3 class="font-bold text-white">Edit Akun</h3></div>
            <div class="bg-activeBg p-6">
                <form method="POST" :action="`/users/${user.id}`" class="space-y-4">
                    ...
                    <div>
                        <label class="font-semibold text-accentDark">Username</label>
                        <input type="text" name="username" class="mt-1 w-full p-2 bg-gray-300 border border-gray-400 rounded-lg text-gray-500 cursor-not-allowed" x-model="user.username" required readonly>
                    </div>
                    <div>
                        <label class="font-semibold text-accentDark">Level</label>
                        <select name="level_user" class="mt-1 w-full p-2 bg-gray-200 border border-gray-300 rounded-lg text-gray-700 focus:ring-2 focus:ring-accentDark" x-model="user.level_user" required>
                            <option value="petugas">Petugas</option>
                            <option value="admin">Admin</option>
                        </select>
                    </div>
                    <div>
                        <label class="font-semibold text-accentDark">Password (Isi jika ingin ganti)</label>
